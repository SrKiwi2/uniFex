...
- Default profile points at the **production** DB (`virtual.uap.edu.bo:5432/v2_fexpo_uap`) and reads secrets (`DB_PASSWORD`, `API_KEY`, `JWT_SECRET`, `ADMIN1_PASSWORD`, `ADMIN2_PASSWORD`) from env vars with **no defaults** — it won't boot without them and must not be run carelessly. `PASARELA_*` vars are optional (pasarela module parked, unwired from the SPA). See `DEPLOY.md` for the full runbook. Always develop with the `dev` profile (`application-dev.properties` has all secrets baked in locally, so no env vars needed).
